...
class TestLikes(unittest.IsolatedAsyncioTestCase):
    async def test_creating_like_on(self):
        ...
        like = await Like.get(1, "comment_id", "topic_id", pool)
        like_on = LikeOn(1, "comment_id", "topic_id", 10, pool)
    ...
    async def test_creating_like_off(self):
        ...
        like = await Like.get(2, "id_comment", "id_topic", pool)
        like_off = LikeOff(2, "id_comment", "id_topic", 10, pool)
    ...
    async def test_switching_on_to_off(self):
        ...
        like_on = LikeOn(2, "id_comment", "id_topic", 10, pool)
        like = Like(like_on)
    ...
    async def test_switching_off_to_on(self):
        pool = AsyncMock()
        pool.fetch = AsyncMock()
        like_on = LikeOff(2, "id_comment", "id_topic", 10, pool)
        like = Like(like_on)
        self.assertTrue(isinstance(like.like, LikeOff))
        self.assertEqual(like.is_on(), False)
        self.assertEqual(like.get_total_likes(), 10)
        ...
        self.assertEqual(mock.call(
            "INSERT INTO likes (user_id, comment_id, topic_id) "
            "VALUES (2, 'id_comment', 'id_topic');"),
                         pool.fetch.mock_calls[0])
